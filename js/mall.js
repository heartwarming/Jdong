...
//在我的商城上
$('.my-function,.my-function-panel').hover(
	function () {
		var locatOffset = $('.my-function').offset();
		$('.my-function-panel').show();
		$('.my-function-panel').css('left', locatOffset.left + "px");
		$('.my-function-panel').css('top', (locatOffset.top + 35) + "px");
		$('.my-function').addClass("nav-pull-down-action");
		$('.my-function').children('a').css("border-left", "1px solid #fff");
	},
	function () {
		$('.my-function-panel').hide();
		$('.my-function').children('a').css("border-left", "1px solid #ccc");
		$('.my-function').removeClass("nav-pull-down-action");
	}
);
//幻灯片
var nowKey = 1;
...
$(function () {
	var $offset = 0;
	$offset = $("#shortcut").height() + 12;
	$('.elevator_list li.common a').click(function (event) {
	...
	$('body').scrollspy({ target: '#shortcut', offset: $offset + 2 });
});
